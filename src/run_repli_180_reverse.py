...
def process_single_instance(file_path: Path, output_csv: Path, config: dict):
    try:
        ...
        final_solution = iterated_local_search(instance, bs_solution, config['max_iterations'], 
                                                        config['max_non_improving'], config['initial_temp'], 
                                                        config['cooling_rate'], remaining_runtime,
                                                        )
...
def run_horizon_batch(horizon: int, base_dir: Path, config: dict):
    ...
    if output_csv.exists():
        try:
            ...
            logger.error(f"Could not parse existing results CSV: {read_err}. Starting fresh.")
    else:
        csv_headers = [
    ...
    logger.info(f"STATUS | Total scanned: {len(instance_files)} | Completed found: {len(completed_instances)} | Left to run: {len(runnable_files)}")

    if not runnable_files:
        logger.info(f"All instances for Horizon {horizon} are already processed. Skipping batch.")
        return

    runnable_files = runnable_files[::-1]

    start_batch_time = time.time()

    # SEQUENTIAL LOOP: Processes files one by one
    for idx, file_path in enumerate(runnable_files, 1):
        logger.info(f"[{idx}/{len(runnable_files)}] Starting processing cycle...")
